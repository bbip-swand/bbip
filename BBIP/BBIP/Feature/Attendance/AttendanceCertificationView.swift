...
struct AttendanceCertificationView: View {
    ...
    @State private var showAttendanceDone: Bool = false // State to control navigation
    ...
    var body: some View {
        NavigationStack { // Add NavigationStack to enable navigation
            VStack(spacing: 0) {
                ...
                // NavigationLink to navigate to AttendanceDoneView when showAttendanceDone becomes true
                NavigationLink(destination: AttendanceDoneView(), isActive: $showAttendanceDone) {
                    EmptyView()
                }
                
                MainButton(text: "파이트!") {
                    if viewModel.isRight { 
                        showAttendanceDone = true // Trigger navigation
                    } else {
                        // Handle incorrect code case here, show warning or error message
                    }
                }
                .padding(.bottom, 22)
            }
