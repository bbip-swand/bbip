        ...
        provider.requestPublisher(.getUserInfo)
            .tryMap{ response in
                guard (200...299).contains(response.statusCode) else {
                    ...
                    throw NSError(
                        domain: "Get User Info Fail",
                        code: response.statusCode,
                        userInfo: [NSLocalizedDescriptionKey: "[Get user infodatasource] getUserInfo() failed with status code \(response.statusCode)"]
                    )
                }
                return response.data
            }
